...
export function WarrantyProvider({ children }) {
  ...
  const fetchMyWarranties = useCallback(async (token) => {
    ...
    try {
      ...
      const list =
        data?.rows ?? data?.warranties ?? (Array.isArray(data) ? data : []);
  ...
  const createWarranty = async (payload, token) => {
    ...
    try {
      ...
      Alert.alert("Thành công", "Gửi yêu cầu bảo hành thành công");
      return created;
    } catch (err) {
      Alert.alert(
        "Lỗi",
  ...
  const validateWarranty = async (id, payload, token) => {
    ...
    try {
      ...
      Alert.alert("Thành công", "Xác thực bảo hành thành công");
      return updated;
    } catch (err) {
      Alert.alert(
        "Lỗi",
